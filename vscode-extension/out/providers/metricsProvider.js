...
class MetricsProvider {
    ...
    async showMetrics() {
        ...
        const panel = vscode.window.createWebviewPanel('revolutionaryUIMetrics', 'Revolutionary UI Metrics', vscode.ViewColumn.One, {
            enableScripts: true
        });
        panel.webview.html = this.getWebviewContent(metrics);
    }
    async calculateProjectMetrics(workspaceUri) {
        const metrics = {
            totalFiles: 0,
            filesWithFactory: 0,
            totalComponents: 0,
